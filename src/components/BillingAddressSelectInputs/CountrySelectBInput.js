...
const CountrySelectBInput = () => {

    const { selectBCountry, setSelectBCountry, setSelectBDivision } = useContext(BILLING_CONTEXT);
    ...
    const handleChoose = (singleCountry) => {
        setSelectBCountry(singleCountry);
        setSelectBDivision(null);
        setFilter([])
        setListShow(false);
    }
    ...
    return (
        <div className="single_select_input">
            ...
            <input onClick={() => setListShow(!listShow)} type="text" onChange={() => setSelectBCountry(selectBCountry)} className='select_input' id='country' placeholder='Please Search' defaultValue={selectBCountry?.name} readOnly />
...
export default CountrySelectBInput;